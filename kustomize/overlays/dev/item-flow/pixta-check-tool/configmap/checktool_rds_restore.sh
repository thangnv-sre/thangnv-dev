...
AUTO_SNAPSHOT_NAME=$(aws rds describe-db-snapshots --profile $PROFILE | grep "${AUTO_SN_PREFIX}" | awk '{print $6}')
SHARED_SNAPSHOT_NAME=$(sed -e 's#.*:\(\)#\1#' <<<"${AUTO_SNAPSHOT_NAME}-shared")
SHARED_SNAPSHOT_ARN="${SNAPSHOT_RESOURCE_NAME}:${SHARED_SNAPSHOT_NAME}"

check_snapshot() {
    while :; do
        aws rds describe-db-snapshots --profile $PROFILE | grep "${AUTO_SNAPSHOT_NAME}" | grep "available"
        STATUS=$?
        if [ $STATUS -eq 0 ]; then
            info "Check automated snapshot successful. DB snapshot Identifier ${AUTO_SNAPSHOT_NAME}"
            break
        else
            sleep 60
        fi
...
restore_from_snapshot() {
    aws rds restore-db-instance-from-db-snapshot \
        ...
        --port 3306 \
        --iops 0 \
        --db-subnet-group-name development \
        --profile $VN_PROFILE
    ...
    while :; do
        ...
        STATUS=$?
        if [ $STATUS -eq 0 ]; then
            info "Restore from $SHARED_SNAPSHOT_NAME successful. DB instance Identifier $DB_INSTANCE_NAME-${SN_NAME}-vpc"
            break
        else
            sleep 60
        fi
...
modify_db_instance() {
    ...
    while :; do
        ...
        STATUS=$?
        if [ $STATUS -eq 0 ]; then
            info "modify db instance successful. DB instance Identifier $DB_INSTANCE_NAME-${SN_NAME}-vpc"
            break
        else
            sleep 60
        fi
...
rename_snapshot_to_staging() {
    ...
    while :; do
        aws rds describe-db-instances --db-instance-identifier $DB_INSTANCE_NAME-vpc --profile $VN_PROFILE | grep "available" && aws rds describe-db-instances --db-instance-identifier $DB_INSTANCE_NAME-vpc --profile $VN_PROFILE | grep "sg-0f91f45ab164780d0" | grep "active"
        STATUS=$?
        if [ $STATUS -eq 0 ]; then
            info "modify db instance successful. DB instance Identifier $DB_INSTANCE_NAME-vpc"
            break
        else
            sleep 60
        fi
    ...
    *)
    ...
    while :; do
        ...
        if [ $LOGIN_STATUS -eq 0 ]; then
            info "db instance update success."
...
delete_db_instance() {
    ...
    while :; do
        ...
        STATUS=$?
        if [ $STATUS -ne 0 ]; then
            info "Delete db instance successful. DB instance Identifier $DB_ID"
            break
        else
            sleep 60
        fi
...
copy_db_snapshot() {
    ...
    while :; do
        ...
        STATUS=$?
        if [ $STATUS -eq 0 ]; then
            info "copy automated snapshot successful. DB snapshot Identifier for sharing ${SHARED_SNAPSHOT_NAME}"
            break
        else
            sleep 60
        fi
...
delete_shared_snapshot() {
    ...
    while :; do
        ...
        STATUS=$?
        if [ $STATUS -ne 0 ]; then
            info "delete shared snapshot successful. DB snapshot Identifier ${SHARED_SNAPSHOT_NAME}"
            break
        else
            sleep 60
        fi
...
add_tags() {
    ...
    for tags in $TAGS_VAL; do
        ...
        if [ $? -eq 0 ]; then
            info "add tags successful. key:$key, value:$value"
        else
...
stop_db_instance() {
    ...
    while :; do
        ...
        STATUS=$?
        if [ $STATUS -eq 0 ]; then
            info "stop db instance successful. DB instance Identifier $DB_INSTANCE_NAME-vpc"
            break
        else
            sleep 60
        fi
